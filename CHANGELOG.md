...
- Toast feedback for the copy buttons (the in-pill "copy secret" icon and
  the output panel's "copy to clipboard" icon): a bottom-center status
  toast announces success or failure, alongside the existing icon-swap-
  to-checkmark cue. Announced via `aria-live="polite"` so screen readers
  pick it up too. Copy failures (e.g. clipboard permission denied), which
  were previously swallowed silently, now surface an error toast.
- New `copied`/`copyFailed` i18n keys, translated across all 27 UI
  languages.
