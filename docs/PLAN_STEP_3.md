...
- `NewRedis(dsn)` — parse via `redis.ParseURL` (`redis://[:password@]host:port/db`), ping on construction. Amendment 2026-07-18: after the ping, read `INFO server` and reject servers older than 6.2 with a clear error (GETDEL requirement — otherwise every retrieval fails at runtime); skip the check when INFO is unavailable (managed offerings may restrict it).
